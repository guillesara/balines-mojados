...
xhr.open("POST", "/mp/status", true);
xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
xhr.send(`payment_id=${queryParams.get('payment_id')}&status=${queryParams.get('status')}`);
xhr.onreadystatechange = (e) => {
    if (xhr.readyState !== 4) return setStatus(false, "Error en la pagina", "Lo sentimos, parece que ha ocurrido un error en la página web. Por favor, póngase en contacto con un administrador para que podamos solucionar el problema lo antes posible. Gracias por su comprensión.");
